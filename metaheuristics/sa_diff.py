...
def generate_candidate(best, candidates):
    ...
    while tries < 1000:
        ...
        if new.mapping != best.mapping and new.mapping not in candidates:
...
@timeit
def run_sa_diff(NRUN, NITER, t_max, cooling_rate):
    random.seed(64)
    np.random.seed(64)
    log = Logbook()
    ...
    for r in range(NRUN):
        ...
        for i in range(NITER):
            print("Iteration %i" % i)
            candidate = generate_candidate(s, candidates)
...
if __name__ == '__main__':
    ...
    t_max = 1000000000
    cooling_rate = 0.96
    print(run_sa_diff(NRUN, NITER, t_max, cooling_rate))
