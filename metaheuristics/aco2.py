...
def calculate_new_probability(task, info, alpha, beta):
    modified = [(i ** alpha) * (j ** beta) if j > 0 else (i ** alpha) * (1 ** beta) for i, j in zip(task, info)]
    ...
    for r in range(NRUN):
        ...
        for g in range(NGEN):
            ...
            if switch:
                ...
                pop = []
                search = []
                while len(search) < ants:
                    test = copy.deepcopy(best)
                    test[random.randint(0, len(test) - 1)] = random.randint(0, CORE_NUMBER - 1)
                    if test not in search:
                        search.append(test)

                #fitnesses = list(toolbox.map(toolbox.evaluate, search))
                #for ind, fit in zip(search, fitnesses):
                #    ind.fitness.values = fit
                #    print(fit)

                pop.extend(search)
            #print(sorted([i.fitness.values[0] for i in pop]))
            #print(pheromones)
            else:
...
if __name__ == '__main__':
    ...
    print(run_aco(pool, 10, 100, 2, 2, 0.2, 100))
