...
class Task:
    ...
    def set_dest_core(self, dest_core):
        self.dest_core_ = dest_core

    def get_dest_core(self):
        return self.dest_core_

    def display(self):
        return [self.id_, self.period_, self.comp_time_, self.message_size_, self.task_dest_,
                self.priority_, self.schedulable_]

    def save(self, f):
        f.write("%i, %i, %i, %i, %i\n" % (self.id_, self.period_, self.comp_time_,
                                              self.message_size_, self.task_dest_))

    def __str__(self):
        return "%i, %i, %i, %i, %i" % (self.id_, self.period_, self.comp_time_,
                                           self.message_size_, self.task_dest_)
